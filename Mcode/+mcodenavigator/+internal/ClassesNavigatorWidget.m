classdef ClassesNavigatorWidget < mcodenavigator.internal.TreeWidget
    ...
    methods
        ...
        function out = setupTreeContextMenu(this, node, nodeData)
            ...
            fileShellName = mcodenavigator.internal.Utils.osFileBrowserName;
        ...
        function out = labelForProperty(this, defn, klassDefn) %#ok<INUSL>
            ...
            access = mcodenavigator.internal.Utils.propertyAccessLabel(defn.GetAccess, ...
                defn.SetAccess);
            if ~isempty(access)
                access = ['[' access ']'];
            end
        ...
        function markDirty(this, node)
            markDirty@mcodenavigator.internal.TreeWidget(this, node);
        ...
        function out = labelForMethod(this, defn)
            ...
            if ~isequal(defn.Access, 'public')
                items{end+1} = mcodenavigator.internal.Utils.memberAccessLabel(defn.Access);
            end
        ...
        function showInheritanceForClass(this, className) %#ok<INUSL>
            ...
            inheritanceWidget = mcodenavigator.internal.ClassInheritanceWidget(classDefn);
...
out = '';
if fid == -1
    warning('apj:mcodenavigator:FileError', 'Could not read file %s: %s', ...
        file, msg);
    return;
end
...
switch nodeData.type
    case 'class'
        mcodenavigator.methodsview2(nodeData.name);
    otherwise
        % Shouldn't get here
...
if isequal(nodeData.type, 'class')
    ...
    if strfind(w, 'is a built-in')
        ...
        mcodenavigator.internal.Utils.guiRevealFileInDesktopFileBrowser(w);
    end
elseif (nodeData.isFile)
    mcodenavigator.internal.Utils.guiRevealFileInDesktopFileBrowser(nodeData.path);
else
    % NOP
end
end
